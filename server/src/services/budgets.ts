...
async function computeObservedAmount(
  ...
  if (policy.metric !== "billed_cents" && policy.metric !== "premium_requests") return 0;
  ...
  const sumColumn = policy.metric === "premium_requests"
    ? costEvents.premiumRequests
    : costEvents.costCents;
